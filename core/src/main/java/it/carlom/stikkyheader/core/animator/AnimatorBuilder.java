...
public class AnimatorBuilder {
  ...
  public AnimatorBuilder() {
    mSetAnimatorBundles = new HashSet<AnimatorBundle>(2);
  }
   ...
   * @param velocityParallax
   *          the velocity to apply to the view in order to show the parallax
   *          effect. choose a velocity between 0 and 1 for better results
   * @return
   */
  public AnimatorBuilder applyVerticalParallax(View viewToParallax, float velocityParallax) {
    ...
    addAnimator(
        AnimatorBundle.create(AnimatorBundle.TypeAnimation.PARALLAX, viewToParallax, null, 0f, -velocityParallax));
  ...
  /**
   * @deprecated 如果你使用控件的中心点作为位移的起点和终点, 那么缩放根本不会对位移产生影响,此方法废弃;
    ...
    for (AnimatorBuilder.AnimatorBundle animatorBundle : mSetAnimatorBundles) {
      ...
      switch (animatorBundle.mTypeAnimation) {
        ...
        case SCALEXY:
          ...
          break;
        case FADE:
          StikkyCompat.setAlpha(animatorBundle.mView, valueAnimation); // TODO
                                                                       // performance
                                                                       // issues?
          break;
        case TRANSLATIONX:
